...
A parser resolves; it does not report, and reporting is what the derived index exists for. Three
things are reported:
...
Those last two are recovered by **differential parses**, not by source rules: mask one `#` with a
private-use code point and read the block again (what comes back with the rest of the line
attached was cut by a comment), and rename one occurrence of a key and read again (if the key is
*still* at the root, it was written twice). This is the point of the change — neither probe needs
to know whether a scalar was written plain, or where the mapping's root indentation is, which is
what round 30 of #156 got wrong.
