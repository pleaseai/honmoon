...
description: "PR #242 (issue #126 stage 1) pins load_or_create_machine_salt's adoption contract in hook.rs with 4 new tests plus doc comments citing 4 pre-existing tests as covering the rest of the boundary — verified all 8 claims true, no gap, no weak assertion, no tag collision"
metadata:
  type: project
---
...
32-byte key; adoption re-tightens to 0600.
...
The doc comment on `load_or_create_machine_salt` cites 4 *pre-existing* tests
as covering the rest of the boundary (absent/short/unreadable file, 0600
re-tightening): `machine_salt_persists_and_is_stable`,
...
this PR's doc comment made a bounded, falsifiable claim (named specific
tests) rather than an unbounded one, which is why verifying it cost one pass
