...
description: honmoon documents PostgreSQL runtime internal timeouts (DRAIN_TIMEOUT, pause_timeout, REFUSAL_ORDER_TIMEOUT) in ADR-0007's Consequences, not in README — established precedent, not an omission to flag
metadata:
  type: project
---

`crates/honmoon-proxy/src/runtime/postgres.rs` has several internal timing constants
(`DRAIN_TIMEOUT` = 30s, `pause_timeout`, and as of PR #112 `REFUSAL_ORDER_TIMEOUT` = 30s) that
...
**Why:** Checked while reviewing PR #112 (pipelined-refusal ordering barrier, `REFUSAL_ORDER_TIMEOUT`).
