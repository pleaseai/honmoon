...
| `crates/honmoon-cli/` | `honmoon` binary — run/gateway/join; `gateway` runs proxy + management API; `hook` is the Claude Code plugin's command-transport backend (stdin hook JSON → `honmoon-core` redaction → stdout verdict) | `src/main.rs`, `src/hook.rs` | `honmoon-core`, `honmoon-proxy`, `honmoon-mgmt`, `tokio`, `clap`, `serde_json`, `hmac`, `sha2` | — (binary) |
