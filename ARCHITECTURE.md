...
- `crates/honmoon-core/src/engine.rs` — `decide()` / `decide_explained()` / `decide_pii_audit_only()` (detect mode, PII-caused verdicts held back): CEL rule evaluation + egress matching.
...
|--------|---------|-----------|------------|-------------|
| `crates/honmoon-core/` | Policy model + decision `engine` (`decide()`/`decide_explained()`/`decide_pii_audit_only()`): CEL rules + egress matching; `audit` (AuditLog/AuditEvent); `protocols` (PostgreSQL/K8s parsers); content-aware detection + redaction: `pii` (Tier-1 PII), `secret_detect` (API keys/tokens), `redact` (joins both into the reversible `secret_tokenizer`) | `src/lib.rs`, `src/engine.rs`, `src/audit.rs`, `src/protocols.rs`, `src/pii.rs`, `src/secret_detect.rs`, `src/redact.rs`, `src/secret_tokenizer/` | `serde`, `serde_json`, `serde_yaml`, `thiserror`, `time`, `cel`, `regex`, `aho-corasick`, `hmac`, `sha2` | `honmoon-proxy`, `honmoon-mgmt`, `honmoon-cli` |
...
| `crates/honmoon-cli/` | `honmoon` binary — run/gateway/join; `gateway` runs proxy + management API; `policy validate` loads a policy through the same loader and exits, binding nothing; `hook` is the Claude Code plugin's command-transport backend (stdin hook JSON → `honmoon-core` redaction → stdout verdict) | `src/main.rs`, `src/hook.rs` | `honmoon-core`, `honmoon-proxy`, `honmoon-mgmt`, `tokio`, `clap`, `serde_json`, `hmac`, `sha2` | — (binary) |
| `packages/claude-plugin/` | Claude Code plugin (`.claude-plugin/plugin.json` + `hooks/hooks.json`): `PreToolUse`/`PostToolUse`/`UserPromptSubmit` command hooks that shell out to `honmoon hook` for client-side secret/PII redaction (transcript hygiene) | `.claude-plugin/plugin.json`, `hooks/hooks.json`, `scripts/redact.sh` | `honmoon` binary (runtime, via PATH/`HONMOON_BIN`) | — (Claude Code plugin) |
