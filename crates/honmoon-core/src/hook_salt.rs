...
//! same machine key. Each reads it from its own `$HOME/.honmoon/hook-salt` (with
//! no `HOME`, a working-directory-relative `.honmoon`), so two processes on one
//! host reading one file get one key and agree. Any arrangement that leaves them
//! holding different bytes does not — separate hosts, a container with its own
