...
| TD-003 | phase-1-http-egress-mvp | Linux implemented for an **unprivileged** child (#36, ADR-0005: empty user+network namespace, proxy bridged over a Unix socket). root/CAP_SYS_ADMIN still bypass it and run downgrades to advisory (fail-open) when namespaces are unavailable; macOS and every other platform still only set proxy env vars, so a child that ignores them bypasses policy there, closed by the Seatbelt profile in #69 | High | 2026-06-20 |
