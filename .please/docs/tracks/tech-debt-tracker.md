...
| TD-003 | phase-1-http-egress-mvp | **Linux resolved** (#36, ADR-0005: empty user+network namespace, proxy bridged over a Unix socket). macOS and every other platform still only set proxy env vars, so a child that ignores them bypasses policy there; closed by the Seatbelt profile in #69 | Medium | 2026-06-20 |
