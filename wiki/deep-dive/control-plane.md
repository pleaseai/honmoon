...
with `script-src 'self'` and `connect-src 'self'`, so only the bundle this origin served runs and a
credential a script did read has nowhere to send itself, while `frame-ancestors 'none'` and
`X-Frame-Options: DENY` stay as they were
([lib.rs:682-771](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-mgmt/src/lib.rs#L682-L771)).
