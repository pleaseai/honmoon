...
Both `honmoon run` and `honmoon gateway` start it; see [Quick Start](/getting-started/quick-start).
:::
...
| `inspect_body` | Buffer, decode, scan for PII, decide, forward or block | [mitm.rs:614-640](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-proxy/src/mitm.rs#L614-L640) |
...
```
<!-- Sources: crates/honmoon-proxy/src/mitm.rs:928-973 (handle_request), mitm.rs:296-336 (host_gate), mitm.rs:217-294 (the uninspectable-endpoint refusal), mitm.rs:614-640 (inspect_body), mitm.rs:1036-1045 (should_intercept), crates/honmoon-proxy/src/gateway.rs:177-203 (serve) -->
...
([mitm.rs:614-640](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-proxy/src/mitm.rs#L614-L640)) — see
...
|---------|------------------|-----------------|--------|
| `Allow` | `Allowed` (on the `CONNECT` only) | `Gate::Proceed` → tunnel (`200`) | [mitm.rs:306-318](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-proxy/src/mitm.rs#L306-L318) |
| `Deny` | `Denied` | `403 Forbidden` | [mitm.rs:319-329](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-proxy/src/mitm.rs#L319-L329) |
| `Pause` | `Paused` → `Approved`/`Rejected` | held, then tunnel or `403` (`503` if the queue is full) | [mitm.rs:330-334](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-proxy/src/mitm.rs#L330-L334), [mitm.rs:338-363](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-proxy/src/mitm.rs#L338-L363) |
...
Over a raw tunnel only `http.host`-based rules see facts, so host-level `pause` rules hold there.
...
`honmoon-cli` dispatches six `clap` subcommands, one of them hidden. Two drive the gateway
(`run`, `gateway`), two are tooling that binds no listener (`policy validate`, `hook`), one is a
stub (`join`), and `supervise-sandbox` is an internal Linux helper
([main.rs:36-283](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-cli/src/main.rs#L36-L283), [main.rs:325-383](https://github.com/pleaseai/honmoon/blob/main/crates/honmoon-cli/src/main.rs#L325-L383)).
