...
- **Slowloris.** hyper's `header_read_timeout` has a 30s default, but it arms only when a `Timer`
  is installed, and nothing in this chain installs one. That crosses two dependencies, so each link
  is cited at the version this workspace builds (`Cargo.lock`: hudsucker 0.24.1, hyper-util 0.1.20,
  hyper 1.10.1) rather than left to the reader. Given no server builder, hudsucker makes its own and
  sets only `title_case_headers` and `preserve_header_case` — the crate calls `.timer(…)` nowhere
  ...
  ([hyper time.rs:70-78](https://github.com/hyperium/hyper/blob/v1.10.1/src/common/time.rs#L70-L78)). A connection whose request head never
  completes is therefore held indefinitely. <span class="status-caveat">regression</span>, tracked
  in [#267](https://github.com/pleaseai/honmoon/issues/267).
