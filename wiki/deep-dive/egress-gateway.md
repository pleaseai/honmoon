...
([gateway.rs:1-11](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L1-L11), [gateway.rs:164-201](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L164-L201)).

## At a glance

| Element | Role | Source |
|---------|------|--------|
| `GatewayState` | Shared `Arc`s: policy + `AuditLog` + `ApprovalRegistry` + pause timeout | [gateway.rs:34-59](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L34-L59) |
| `run(policy, addr)` | Bind `addr`, serve forever | [gateway.rs:62-65](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L62-L65) |
| `serve_listener(policy, listener)` | Serve a pre-bound listener (no TOCTOU) | [gateway.rs:71-74](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L71-L74) |
| `serve_listener_with_state` / `serve` | Serve with caller-provided shared state (used by `honmoon gateway`) | [gateway.rs:78-109](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L78-L109) |
| `handle(client, state)` | One connection: parse → authorize → tunnel | [gateway.rs:112-162](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L112-L162) |
| `authorize` | Decide + audit; routes `pause` to the hold | [gateway.rs:169-201](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L169-L201) |
| `hold_for_approval` | Register, wait for human/timeout/disconnect, audit resolution | [gateway.rs:206-](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L206) |
| `read_head` / `canonical_host` | Slowloris-guarded head read; host canonicalization | [gateway.rs](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs) |
...
([gateway.rs:89-162](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L89-L162)).
...
([gateway.rs:169-201](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L169-L201)):

| Verdict | Audit `Decision` | Client response | Source |
|---------|------------------|-----------------|--------|
| `Allow` | `Allowed` | tunnel (`200`) | [gateway.rs:177-186](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L177-L186) |
| `Deny` | `Denied` | `403 Forbidden` | [gateway.rs:187-198](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L187-L198) |
| `Pause` | `Paused` → `Approved`/`Rejected` | held, then tunnel or `403` | [gateway.rs:199](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L199), [gateway.rs:206-](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L206) |
...
([gateway.rs:206-271](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L206-L271), [approval.rs:101-129](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/approval.rs#L101-L129)):
...
rather than growing unbounded ([gateway.rs:218-230](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L218-L230), [approval.rs:61-64](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/approval.rs#L61-L64)), and a
**timeout auto-rejects** a held request so it never hangs forever
([gateway.rs:250-255](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-proxy/src/gateway.rs#L250-L255)). The resolution travels back through
...
([main.rs:131-165](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L131-L165)).
Binding in one place closes the TOCTOU window where another process could steal the port
([main.rs:140-148](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L140-L148)). `run` uses an
...
held by the proxy can be approved from the dashboard ([main.rs:78-128](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L78-L128)). It binds both
listeners up front and uses `tokio::select!` so an unexpected proxy exit surfaces instead of
silently leaving egress filtering down ([main.rs:104-127](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L104-L127)).

| Flag | Default | Purpose | Source |
|------|---------|---------|--------|
| `--addr` | `127.0.0.1:8443` | Egress proxy listen address | [main.rs:39-40](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L39-L40) |
| `--mgmt-addr` | `127.0.0.1:8444` | Management API + dashboard address | [main.rs:42-43](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L42-L43) |
| `--audit-log` | (in-memory only) | Append every verdict to a JSONL file | [main.rs:45-46](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L45-L46), [main.rs:89-95](https://github.com/pleaseai/honmoon/blob/master/crates/honmoon-cli/src/main.rs#L89-L95) |
