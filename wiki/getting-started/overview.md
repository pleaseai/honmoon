...
proxy's loopback port. Neither replaces the filesystem, so Unix sockets that live there —
`/var/run/docker.sock` and friends — stay reachable on both, and anything a local daemon behind one
