...
| **Exit non-zero** | The policy does not load, and the error naming every offending rule is on stderr |
...
change the exit code: the gateway starts on a policy carrying one, so this accepts it too. Set
`RUST_LOG` to see more or less.
...
The policy's own contents are not echoed on success — a count of what loaded, and nothing more.
An endpoint map names the hosts an operator cares most about, and a CI log is not somewhere they
chose to put them. A *rejection* does quote the rules responsible: that text is the diagnosis.
